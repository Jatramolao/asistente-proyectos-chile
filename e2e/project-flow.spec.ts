...
test("creates a project from an idea and keeps inferred facts unvalidated", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Checklist transversal" })).toBeVisible();
  await expect(page.getByRole("heading", { name: /Capital Semilla Emprende Región Metropolitana/i })).toBeVisible();
  ...
  await ageChecklistItem.getByRole("button", { name: "Guardar estado" }).click();
